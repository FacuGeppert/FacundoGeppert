class BrowserStockfishEngine {
  ...
  async init() {
    ...
    for (const url of urls) {
      try {
        await this.tryCreateWorker(url);
        this.ready = true;
        return true;
      } catch (error) {
        lastError = error;
    ...
    throw lastError || new Error("No se pudo cargar Stockfish desde CDN.");
  }
  ...
  tryCreateWorker(url) {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (!settled) {
          settled = true;
          reject(new Error("Timeout cargando Stockfish."));
        }
      }, 9000);
      ...
      worker.onerror = (event) => {
        if (!settled) {
          ...
          reject(new Error(event.message || "Error cargando worker de Stockfish."));
        }
      };
